...
- Effects as data: pure logic enqueues typed `Logic.Effects.*` structs with `Effects.enqueue/2`; the entity owner drains them with `EventSink.emit_pending/1`. `EffectResolver` turns semantic requests into concrete effects, and `EventSink` projects them. Owner-local delivery uses `EventSink.Context`, never ambient `self()` or direct packet sends in interpreters
...
- Mob engagement lifecycle transitions go through `Logic.Engagement`; `Logic.Threat` owns threat values, not victim, tap, combat, death, or reset cleanup
