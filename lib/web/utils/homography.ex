defmodule ThistleTeaWeb.Homography do
  ...
  def find_homography(a, b) do
    ...
    {equations, constants} =
      Enum.reduce(input, {[], []}, fn {{x, y}, {u, v}}, {eq_acc, const_acc} ->
        eq_acc =
          eq_acc ++
            [
              [x, y, 1, 0, 0, 0, -u * x, -u * y],
        ...
        const_acc = const_acc ++ [u, v]
        {eq_acc, const_acc}
      end)

    equations_tensor = Nx.tensor(equations, type: :f32)
    constants_tensor = Nx.tensor(constants, type: :f32)

    solution = Nx.LinAlg.least_squares(equations_tensor, constants_tensor)

    Nx.concatenate([solution, Nx.tensor([1.0], type: :f32)])
    |> Nx.reshape({3, 3})
  end
