defmodule ThistleTea.Game.Entity.Logic.Proficiency do
  ...
  gate equipping weapons and armor, while skills and known spells enforce
  item-template requirements. Fishing grants the fishing-pole subclass bit directly because
  ...
  def from_character(%Character{player: player, internal: internal}) do
    ...
    prof = internal.spellbook |> from_spellbook() |> put_skills(skills)
    %{prof | known_spell_ids: MapSet.union(prof.known_spell_ids, MapSet.new(internal.spells || []))}
  end
  ...
  def can_equip?(%__MODULE__{} = prof, %ItemTemplate{} = template) do
    with :ok <- check_required_skill(prof, template),
         :ok <- check_required_spell(prof, template) do
      check_item_class(prof, template)
    end
  end

  defp check_required_spell(%__MODULE__{known_spell_ids: :all}, _template), do: :ok

  defp check_required_spell(%__MODULE__{known_spell_ids: known}, %ItemTemplate{required_spell: id})
       when is_integer(id) and id > 0 do
    if MapSet.member?(known, id), do: :ok, else: {:error, :no_required_proficiency}
  end

  defp check_required_spell(_prof, _template), do: :ok
