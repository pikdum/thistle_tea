defmodule ThistleTea.Game.Entity.EventSink.ClientProjection do
  ...
  def emit(%Character{} = entity, %Effects.EnchantItem{effect: %{type: :enchant_item}} = effect, context) do
    Context.send(
      context,
      {:enchant_item_permanent, effect.target_guid, effect.spell, effect.effect.misc_value, effect.cast_item_guid}
    )
  ...
  def emit(entity, %Effects.OpenLock{}, _context), do: entity

  def emit(%Character{} = entity, %Effects.PickPocket{target_guid: guid, spell_id: spell_id}, context) do
    Context.send(context, {:pickpocket, guid, spell_id})
    entity
  end
