defmodule ThistleTea.DB.Mangos.QuestStartScript do
  @moduledoc false
  use Ecto.Schema
  ...
  def query(script_ids) when is_list(script_ids) do
    from(script in __MODULE__,
      where: script.id in ^script_ids,
      order_by: [script.id, script.delay, script.priority]
    )
  end
end
