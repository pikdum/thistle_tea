...
Shared leash timing and `creature_groups_entry_limit` composition remain separate missing pieces. Navigation uses the existing Namigator path resolution; exact VMangos walk-hit endpoint clipping is not independently established by this run. This increment does not establish full creature-group or vanilla feature parity.
