defmodule ThistleTea.Game.Entity.Server.GameObject.ChairTest do
  ...
  describe "seat/3" do
    test "selects the nearest slot and applies the configured chair height" do
      ...
      assert {:ok, {seat_x, seat_y, 4.0, orientation}, 5} = Chair.seat(chair, WorldRef.open(0), {10.0, 22.2, 4.0})
    ...
    test "uses the object center when the template has no slots" do
      ...
      assert {:ok, {10.0, 20.0, 4.0, 1.0}, 4} = Chair.seat(chair, WorldRef.open(0), {10.0, 20.0, 4.0})
    end

    test "rejects a player farther than three yards from the nearest slot" do
      chair = chair(slots: 1, position: {10.0, 20.0, 4.0, 0.0})

      assert {:error, :too_far} = Chair.seat(chair, WorldRef.open(0), {14.0, 20.0, 4.0})
    end

    test "rejects players on another map" do
      assert {:error, :not_a_chair} = Chair.seat(chair(), WorldRef.open(1), {10.0, 20.0, 4.0})
    end
  end
  ...
  defp chair(opts \\ []) do
    ...
    %GameObject{
      ...
      internal: %Internal{world: %WorldRef{map_id: 0}, chair: %ChairConfig{slots: slots, height: height}},
