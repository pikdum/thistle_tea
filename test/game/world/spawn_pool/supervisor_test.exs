defmodule ThistleTea.Game.World.SpawnPool.SupervisorTest do
  use ExUnit.Case, async: false

  alias ThistleTea.Game.World.SpawnPool.Supervisor, as: SpawnPoolSupervisor

  describe "start_child/2" do
    test "routes unrelated pools across supervisor partitions" do
      [{first_key, first_supervisor}, {second_key, second_supervisor}] = distinct_partitions()

      assert {:ok, first_pid} = SpawnPoolSupervisor.start_child(first_key, {Agent, fn -> :first end})
      assert {:ok, second_pid} = SpawnPoolSupervisor.start_child(second_key, {Agent, fn -> :second end})
      ...
      assert :ok = SpawnPoolSupervisor.terminate_child(first_pid)
      assert :ok = SpawnPoolSupervisor.terminate_child(second_pid)
    end
  end
  ...
  defp partition(key) do
    {:via, PartitionSupervisor, {SpawnPoolSupervisor, key}}
    |> GenServer.whereis()
  end
